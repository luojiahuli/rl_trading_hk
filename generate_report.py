...
from src.data.sector_map import _SECTOR_STOCK_MAP_HK
...
def fetch_all_data() -> dict[str, pd.DataFrame]:
    """获取所有股票的日线数据并计算指标"""
    all_codes = []
    for sector, codes in _SECTOR_STOCK_MAP_HK.items():
        for code in codes:
...
def main():
    print(f"HK Backtest Report Generator")
