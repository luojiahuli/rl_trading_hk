...
class FeishuPushAgent(BaseAgent):
    ...
    def _build_content(self, ctx: AgentContext) -> dict:
        lines = [[{"tag": "text", "text": f"📊 港股市场: {ctx.regime} | 股票: {len(ctx.stock_pool)}只 | 信号: {len(ctx.rl_signals)}个 | 回测: {len(ctx.backtest_results)}次"}]]
        ...
        return {"zh_cn": {"title": f"🚀 港股量化日报 {ctx.date}", "content": lines}}
