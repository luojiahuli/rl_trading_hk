"""数据获取 Agent — 港股版（yfinance）"""
import pandas as pd
from ..agents.base import AgentContext, BaseAgent
from ..data.fetcher import fetch_stock_daily
from ..data.indicators import compute_indicators
from config import START_DATE, END_DATE

from ..data.sector_map import _SECTOR_STOCK_MAP_HK


class DataFetchAgent(BaseAgent):
    name = "data_fetch"
    description = "按热门板块获取港股数据和指标"

    def execute(self, context: AgentContext) -> AgentContext:
        stock_pool = []
        market_data = {}
        indicators = {}

        for hs in context.hot_sectors:
            sector = hs["sector"]
            stocks = hs.get("stocks", [])
            if not stocks:
                stocks = _SECTOR_STOCK_MAP_HK.get(sector, [])

            for code in stocks[:6]:
                try:
                    df = fetch_stock_daily(f"{code}.HK", START_DATE, END_DATE)
                except Exception:
                    df = pd.DataFrame()

                if df is None or len(df) < 30:
                    context.warnings.append(f"{code}.HK: 数据不足")
                    continue

                try:
                    df = compute_indicators(df)
                except Exception:
                    continue

                full_code = f"{code}.HK"
                market_data[full_code] = df
                stock_pool.append(full_code)

                if len(df) > 0:
                    last = df.iloc[-1]
                    indicators[full_code] = {
                        "close": last.get("close", 0),
                        "pct_chg": last.get("pct_chg", 0),
                        "volume_ratio": last.get("volume_ratio", 1),
                        "rsi_14": last.get("rsi_14", 50),
                        "price_position": last.get("price_position", 0.5),
                        "atr": last.get("atr", 0),
                    }

        context.stock_pool = stock_pool
        context.market_data = market_data
        context.indicators = indicators
        context.warnings.append(f"港股数据获取完成: {len(stock_pool)} 只股票")
        return context
