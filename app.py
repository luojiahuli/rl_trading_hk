...
def answer_question(question: str, history: list):
    ...
    if not ctx:
        ...
        parts = []
        if ctx.hot_sectors:
            parts.append(f"今日港股热门板块: {', '.join(s['sector'] for s in ctx.hot_sectors[:5])}")
        if ctx.rl_signals:
...
with gr.Blocks(title="港股量化交易系统") as demo:
    gr.Markdown("# 🚀 港股量化交易系统\n基于多智能体架构的港股每日动态机会点挖掘")
    ...
    def on_run(date_str):
        yield "⏳ 正在执行港股分析管线...", "", "", {"status": "running"}
        try:
            report, viz_h, pos_df, summary = run_analysis(date_str)
            yield "✅ 港股分析完成", report, viz_h, pos_df, summary
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="港股量化交易系统 Web UI")
